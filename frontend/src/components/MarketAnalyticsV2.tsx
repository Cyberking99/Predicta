...
export function MarketAnalyticsV2() {
  ...
  const {
    ...
    queryFn: async () => {
      const markets = await getMarkets(50, 0, "totalVolume", "desc");
      return markets;
    },
    enabled: true,
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const market = await getMarketById(selectedMarketId.toString());
      return market;
    },
          ...
          <CardContent>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {marketsList.map((market) => (
                <div
                  key={market.id}
                  className={`p-3 rounded-lg border cursor-pointer transition-colors ${selectedMarketId === market.id
