...
const Header = ({ walletBalance, setWalletBalance }) => {
  ...
  const {
    ...
    enabled: !!address && isConnected && !!CUSD_TOKEN_ADDRESS,
  ...
  useEffect(() => {
    if (stimBalanceError) {
      console.error('❌ CUSD Balance Error:', stimBalanceError);
    }
    if (usdcBalanceError) {
      console.error('❌ USDC Balance Error:', usdcBalanceError);
    }
    if (stimBalanceData) {
      console.log('✅ CUSD Balance Data:', stimBalanceData);
    }
    if (usdcBalanceData) {
  ...
  const isBalanceLoading = isCusdLoading || isUsdcLoading;
