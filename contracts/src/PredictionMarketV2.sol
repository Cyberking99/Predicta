...
contract PredictionMarketV2 is AccessControl, Ownable, Pausable, ReentrancyGuard {
    ...
    modifier marketExists(uint256 _marketId) {
        _marketExists(_marketId);
        _;
    }

    modifier marketNotResolved(uint256 _marketId) {
        _marketNotResolved(_marketId);
        _;
    }

    modifier marketNotInvalidated(uint256 _marketId) {
        _marketNotInvalidated(_marketId);
        _;
    }

    modifier marketActive(uint256 _marketId) {
        _marketActive(_marketId);
        _;
    }

    modifier marketEnded(uint256 _marketId) {
        _marketEnded(_marketId);
        _;
    }
    
    function _marketExists(uint256 _marketId) internal view {
        require(_marketId < marketCount, "Market does not exist");
    }
    
    function _marketNotResolved(uint256 _marketId) internal view {
        require(!markets[_marketId].resolved, "Market already resolved");
    }

    function _marketNotInvalidated(uint256 _marketId) internal view {
        require(!markets[_marketId].invalidated, "Market invalidated");
    }

    function _marketActive(uint256 _marketId) internal view {
        require(block.timestamp < markets[_marketId].endTime, "Market ended");
    }

    function _marketEnded(uint256 _marketId) internal view {
        require(block.timestamp >= markets[_marketId].endTime, "Market ended");
    }
    ...
    function createMarketWithToken(
        ...
        CreateMarketParams memory params = CreateMarketParams({
            ...
            optionDescriptions: _optionDescriptions,
            duration: _duration,
            category: _category,
            marketType: _marketType,
    ...
    function _createMarket(CreateMarketParams memory params) internal returns (uint256) {
        require(bytes(params.question).length > 0, "Question required");
        require(params.optionNames.length >= 2, "At least 2 options required");
        require(params.optionNames.length == params.optionDescriptions.length, "Options mismatch");
        require(params.duration >= MIN_MARKET_DURATION && params.duration <= MAX_MARKET_DURATION, "Invalid duration");
        require(params.initialLiquidity > 0, "Initial liquidity required");
        require(whitelistedTokens[params.bettingToken], "Token not whitelisted");
        ...
        require(
            IERC20(params.bettingToken).transferFrom(msg.sender, address(this), params.initialLiquidity),
            "Transfer failed"
        );

        // Calculate LMSR B parameter
        uint256 lmsrB = _calculateLMSRB(params.initialLiquidity, params.optionNames.length);
        ...
        markets[marketId] = Market({
            ...
            validated: false,
            winningOptionId: 0,
            optionCount: params.optionNames.length,
            totalVolume: 0,
            lmsrB: lmsrB,
            adminInitialLiquidity: params.initialLiquidity,
            userLiquidity: 0,
            platformFeesCollected: 0,
            adminLiquidityClaimed: false,
            earlyResolutionAllowed: params.earlyResolutionAllowed,
            bettingToken: params.bettingToken
        });

        // Initialize options with equal shares from initial liquidity
        uint256 sharesPerOption = params.initialLiquidity / params.optionNames.length;
        for (uint256 i = 0; i < params.optionNames.length; i++) {
            options[marketId][i] = Option({
                name: params.optionNames[i],
                description: params.optionDescriptions[i],
                totalShares: sharesPerOption,
                totalVolume: 0,
                currentPrice: 1e18 / params.optionNames.length, // Equal initial price
                isActive: true
            });
        }

        // Setup free market if applicable
        if (params.marketType == MarketType.FREE && params.freeParams.maxFreeParticipants > 0) {
            uint256 totalPrizePool = params.freeParams.maxFreeParticipants * params.freeParams.tokensPerParticipant;
            freeMarketConfigs[marketId] = FreeMarketConfig({
                maxFreeParticipants: params.freeParams.maxFreeParticipants,
                tokensPerParticipant: params.freeParams.tokensPerParticipant,
            ...
            emit FreeMarketConfigSet(
                marketId, params.freeParams.maxFreeParticipants, params.freeParams.tokensPerParticipant, totalPrizePool
            );
        }

        categoryMarkets[params.category].push(marketId);
        marketsByType[params.marketType].push(marketId);

        emit MarketCreated(
            marketId, params.question, params.optionNames, endTime, params.category, params.marketType, msg.sender
        );
        emit BComputed(marketId, lmsrB, params.initialLiquidity, params.optionNames.length);
        ...
        marketNotInvalidated(_marketId)
    {
        ...
        priceHistory[_marketId][_optionId]
        .push(
            PricePoint({
                price: options[_marketId][_optionId].currentPrice, timestamp: block.timestamp, volume: totalCost
            })
        );
        ...
        marketNotInvalidated(_marketId)
    {
        ...
        .push(
            PricePoint({
                price: options[_marketId][_optionId].currentPrice, timestamp: block.timestamp, volume: totalProceeds
            })
        );
    ...
    function _calculateBuyCost(uint256 _marketId, uint256 _optionId, uint256 _quantity)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 _newShares = currentShares + _quantity;
        uint256 _b = markets[_marketId].lmsrB;
    ...
    function _calculateSellProceeds(uint256 _marketId, uint256 _optionId, uint256 _quantity)
        internal
        view
        returns (uint256)
    {
    ...
    function _updatePrice(uint256 _marketId, uint256 _optionId) internal {
        options[_marketId][_optionId].currentPrice =
            _calculatePrice(_marketId, _optionId, options[_marketId][_optionId].totalShares);
    }
    ...
    function validateMarket(uint256 _marketId) external onlyRole(MARKET_VALIDATOR_ROLE) marketExists(_marketId) {
    ...
    {
        require(
            userShares[msg.sender][_marketId][0] > 0 || userShares[msg.sender][_marketId][1] > 0, "Must be participant"
        );
    ...
    function withdrawAdminLiquidity(uint256 _marketId) external nonReentrant marketExists(_marketId) {
    ...
    function withdrawPlatformFeesForToken(address _token, uint256 _amount) external onlyOwner nonReentrant {
        ...
        returns (uint256 winningOptionId, bool disputed, bool validated, address creator, bool earlyResolutionAllowed)
    {
        Market memory market = markets[_marketId];
        return
            (market.winningOptionId, market.disputed, market.validated, market.creator, market.earlyResolutionAllowed);
    }
    ...
    {
        return (hasClaimedWinnings[_marketId][_user], hasClaimedFreeTokens[_marketId][_user]);
    }

    function getMarketLMSRB(uint256 _marketId) external view marketExists(_marketId) returns (uint256) {
        return markets[_marketId].lmsrB;
    }

    function getMarketDisputeStatus(uint256 _marketId) external view marketExists(_marketId) returns (bool) {
